#!/usr/bin/env python3
"""
Populate ar_oh table from cohort data for University of Akron (OH)
"""
...
def populate_ar_oh():
    """Extract analysis-ready data from cohort table and insert into ar_oh"""
    ...
    # Clear existing ar_oh data
    print('Clearing existing ar_oh data...')
    cursor.execute('DELETE FROM ar_oh')
    conn.commit()
    
    # Insert analysis-ready data from cohort
    print('Populating ar_oh from cohort data...')
    ...
    print(f'Successfully inserted {rows_inserted} rows into ar_oh')
    
    # Verify
    cursor.execute('SELECT COUNT(*) FROM ar_oh')
    ar_count = cursor.fetchone()[0]
    print(f'ar_oh now has {ar_count} rows')
